package com.accenture.cpaas.enabler.dcm.corradodue.masteraip.be.processor;
...
import com.accenture.cpaas.enabler.dcm.corradodue.masteraip.be.util.EntityBean;
...
public class SerializeRequestProcessor  extends GenericProcessor {
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        CorradodueMasteraip request = exchange.getIn().getBody(CorradodueMasteraip.class);
        
        String rootSchema = CorradodueMasteraip.class.getSimpleName();
    ...
    private void ricorsivo(String rootSchema, Object request, List<EntityBean> entityBeans, String tableName, String tableRoot, Header header, String randomId, String root) throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, JsonProcessingException, ClassNotFoundException, InstantiationException {
        Class<?> clazz = Class.forName("com.accenture.cpaas.enabler.dcm.corradodue.masteraip.be.access.entity."+ rootSchema); 
                	...
                	if (method.isAnnotationPresent(JsonProperty.class)) {
    					...
    					if (method.getReturnType().getName().startsWith("java.util.List")) {
                            if (StringUtils.substringAfterLast(tableRoot, "_") != null && StringUtils.substringAfterLast(tableRoot, "_").equalsIgnoreCase(method.getName().substring(3))) {
                             //   ricorsivolist(method.getName().substring(3), method.invoke(request), entityBeans, tableRoot, tableRoot, header, randomUUID);
...
//        String tmpRootSchema = rootSchema.replaceAll("(.)([A-Z])", "$1_$2").toLowerCase(); 
//        String tmpTableRoot = tableRoot.replaceAll("(.)([A-Z])", "$1_$2").toLowerCase();
...
//        if (tmpTableRoot.equals(tmpRootSchema)) {
//        
//                entityBeans.add(new EntityBean(tmpRootSchema, objectMapper.writeValueAsString(t)));
//        } else {
//            if (StringUtils.substringAfterLast(tmpTableRoot, "_") != null && StringUtils.substringAfterLast(tmpTableRoot, "_").equalsIgnoreCase(tmpRootSchema)) {
//                entityBeans.add(new EntityBean(tmpTableRoot, objectMapper.writeValueAsString(t)));
//            } else {
//                tmpRootSchema = tmpTableRoot + "_" + tmpRootSchema;
//                entityBeans.add(new EntityBean(tmpRootSchema, objectMapper.writeValueAsString(t)));
//            }
//        }
    }
